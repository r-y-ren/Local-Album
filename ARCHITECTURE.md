...
| `:opencv` | `opencv/java/` | OpenCV 5.0 上游 Java 绑定 + native `jniLibs`。生成代码不改动，仅替换了构建脚本（历史备份文件已于 2026-08 治理移除） |
...
| 构建脚本           | 维护性     | **Version Catalog 已建立（2026-08 治理）**：`gradle/libs.versions.toml`（25 versions / 34 libraries / 5 bundles），app/opencv 依赖全部走 `libs.*` 访问器，版本与迁移前零漂移。遗留：`apply(plugin)` 旧式 DSL 与 root `buildscript` workaround（plugins DSL 迁移有 KGP 约束过滤 bug 复现风险，需独立试验）；AGP 内部 API 依赖；4 条无用依赖已删除；~~`.bak`~~ 已 `git rm` 并补忽略规则 |
| 大文件             | 可维护性   | **`LocalAlbumApp.kt` 已拆分（2026-08 治理）**：2838 → 881 行（-69%），Tab/推荐/设置 Section 已提取至 `ui/screens/`（设置拆为 7 个 Section 文件），导航宿主保留原文件。遗留：`PluginViewModel.kt`（1200+ 行）、`AppDatabase.kt`（1000+ 行迁移链）、`AppContainer.kt`（750+ 行）待拆分评估                                                                                              |
...
| 2026-08 治理   | 依赖精简（删 4 条无用依赖 + Version Catalog 化零漂移）、死代码清理（14 文件约 2300 行 + 50 个未引用 string）、`ModelDownloadManagerV2` 单例收敛 + Mutex 加固、`SettingsRepository` combine 类型安全化、`ScanBenchmarkConfig` 拆分、`LocalAlbumApp.kt` 拆分（2838→881 行）、核心链路 KDoc 15 项 + 魔法值常量化 12 组；审计/清单报告见 `plans/phase2~4` 系列 |
