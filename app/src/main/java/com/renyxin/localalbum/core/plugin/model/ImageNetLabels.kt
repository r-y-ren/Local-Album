...
/**
 * ImageNet 1001 类标签（含 background 类）。
 *
 * 用于 MobileNetV2 分类器的输出映射。
 * 索引 0 为 background，索引 1-1000 为 ImageNet 类别。
 * 来源: TensorFlow Lite MobileNetV2 模型默认标签。
 */
object ImageNetLabels {
    ...
    fun toSceneLabel(index: Int, confidence: Float): String {
        ...
        return when {
            // 自然风景
            label in setOf("lakeside", "seashore", "valley", "cliff", "sandbar", "promontory",
                "alp", "volcano", "coral reef", "geyser", "hay") -> "nature"
            // 海滩
            label in setOf("seashore", "sandbar", "beach wagon") -> "beach"
            // 山
            label in setOf("alp", "volcano", "cliff") -> "mountain"
            // 花
            label in setOf("daisy", "yellow lady's slipper", "corn poppy", "rose", "sunflower") -> "flower"
            label.contains("flower") || label in listOf("rapeseed", "cardoon") -> "flower"
            // 食物
            label in setOf("ice cream", "hotdog", "pizza", "banana", "orange",
                "cheeseburger", "burrito", "guacamole", "carbonara", "trifle",
                "mashed potato", "French loaf", "bagel", "pretzel", "consomme",
                "hot pot", "meat loaf", "potpie") -> "food"
            label in listOf("strawberry", "pineapple", "lemon", "fig", "jackfruit",
                "custard apple", "pomegranate", "Granny Smith") -> "food"
            // 动物
            label in setOf("tabby", "tiger cat", "Persian cat", "Siamese cat",
                "Egyptian cat", "cougar", "lynx", "leopard", "snow leopard",
                "jaguar", "lion", "tiger", "cheetah") -> "animal"
            anyDog(label) -> "animal"
            label in setOf("koala", "giant panda", "lesser panda", "red panda",
                "brown bear", "American black bear", "ice bear", "sloth bear",
                "orangutan", "gorilla", "chimpanzee", "monkey", "baboon") -> "animal"
            // 建筑
            label in setOf("castle", "palace", "monastery", "church", "mosque",
                "triumphal arch", "stupa", "dome", "altar", "library",
                "bookshop", "bakery", "barbershop", "butcher shop", "grocery store") -> "architecture"
            // 交通工具
            label in setOf("airliner", "aircraft carrier", "airship", "warplane",
                "space shuttle", "passenger car", "sports car", "convertible", "jeep",
                "limousine", "minivan", "pickup", "school bus", "trolleybus",
                "bullet train", "steam locomotive", "electric locomotive", "submarine",
                "container ship", "aircraft carrier") -> "vehicle"
            // 人物
            label in setOf("groom", "ballplayer", "scuba diver") -> "people"
            // 夜景/日落
            label in setOf("sundial", "fire screen", "candle", "torch", "spotlight") -> "night"
            // 水
            label in setOf("fountain", "water tower", "water jug", "water bottle",
                "breakwater", "dam", "dock", "pier") -> "water"
            // 文档
            label in setOf("book jacket", "menu", "comic book", "crossword puzzle",
                "envelope", "web site", "notebook") -> "document"
            // 默认
            else -> label
        }
    }

    private fun anyDog(label: String): Boolean {
        val dogs = setOf("Chihuahua", "Japanese spaniel", "Maltese dog", "Pekinese",
            "Shih-Tzu", "Blenheim spaniel", "papillon", "toy terrier", "Rhodesian ridgeback",
            "Afghan hound", "basset", "beagle", "bloodhound", "bluetick",
            "black-and-tan coonhound", "Walker hound", "English foxhound", "redbone", "borzoi",
            "Irish wolfhound", "Italian greyhound", "whippet", "Ibizan hound", "Norwegian elkhound",
            ...
            "kelpie", "komondor", "Old English sheepdog", "Shetland sheepdog", "collie",
            ...
            "Arctic fox", "grey fox")
        return label in dogs || dogs.any { label.contains(it, ignoreCase = true) }
    }
}
