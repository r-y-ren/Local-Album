...
class FaceClusterMaintenanceWorker(context: Context, params: WorkerParameters) : CoroutineWorker(context, params) {
    override suspend fun doWork(): Result {
        ...
        val run = dao.resumableRun() ?: createRun(db)
        val generation = run.generation

        return try {
            if (run.status == MaintenanceRunEntity.STATUS_FINALIZING) {
                return if (dao.publish(generation)) Result.success() else Result.retry()
            }
            var cursor = run.cursorPath?.toLongOrNull() ?: 0L
            var batches = 0
            ...
            dao.markFailed(generation, error.message?.take(240) ?: error.javaClass.simpleName, System.currentTimeMillis())
            Result.retry()
        }
    }
