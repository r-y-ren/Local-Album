...
class SemanticClusterMaintenanceWorker(context: Context, params: WorkerParameters) : CoroutineWorker(context, params) {
    override suspend fun doWork(): Result {
        ...
        val index = semanticDao.getActiveIndexMeta() ?: return Result.success()
        val embeddingDao = db.embeddingDao()
        val reservoir = PriorityQueue<Sample>(compareByDescending<Sample> { it.priority }.thenByDescending { it.row.filePath })
        ...
        } while (page.size == PAGE_SIZE && !isStopped)
        if (isStopped) return Result.retry()

        val vectors = reservoir.mapNotNull { sample ->
            runCatching { sample.row.filePath to EmbeddingCodec.decode(requireNotNull(sample.row.embeddingBlob)) }.getOrNull()
        ...
        val grouped = vectors.groupBy { dominantBucket(it.second) }.toSortedMap().entries.take(MAX_CLUSTERS)
        ...
        grouped.forEachIndexed { clusterIndex, entry ->
            ...
            entry.value.forEach { (_, vector) -> vector.indices.forEach { centroid[it] += vector[it] } }
            centroid.indices.forEach { centroid[it] /= entry.value.size.toFloat() }
            val topPaths = entry.value.asSequence().map { it.first }.sorted().take(TOP_N).toList()
            ...
            topPaths.forEachIndexed { rank, path ->
                members += SemanticClusterMemberEntity(index.spaceId, generation, clusterId, rank, path)
            }
        }
        ...
        return if (semanticDao.replaceAndPublishClusters(generationRow, clusters, members, now)) Result.success() else Result.retry()
    }
