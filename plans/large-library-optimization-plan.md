...
- 冷启动相册恢复已改为“持久化目录快照优先、后台增量校准”：数据库 v28 增加原子发布的目录快照及版本元数据；有历史数据时先展示上次完整相册，扫描中断、失败或进程被杀均不覆盖已提交快照。
- 相册页增加独立的数据同步状态提示，明确区分恢复、检查、更新、已更新和失败；只有没有任何已提交快照的首次扫描才显示阻塞式首次构建页。
- 首次进程启动顺序调整为先恢复快照再触发前台补偿扫描，推荐与统计移出相册首屏关键路径。
...
20. **AI 识别与结果偏好首批代码已完成。** 设置页新增独立入口与显式保存页面，首批开放人物归并严格程度、人物最少成组数、OCR 分析范围、语义搜索严格程度、默认结果数量和精选推荐倾向。配置通过 DataStore 原子保存并进入进程不可变快照；人脸首次/增量规则成组读取同一预设，OCR 可限定截图文档或关闭，语义搜索参数用于下一次查询，推荐倾向用于下一次刷新。人物参数只影响后续归类，不自动破坏已有分组；完全统一仍需用户显式执行人物重新整理。参数边界测试与 Debug 构建已通过。
